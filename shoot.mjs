...
// focus rack: interior vs surface emphasis
const setF=async(v)=>page.evaluate(v=>{const s=document.querySelector('#focus-blend');s.value=v;s.dispatchEvent(new Event('input',{bubbles:true}));},v);
...
// real-data ensemble + peel-back
await page.evaluate(()=>document.querySelector('#datasrc button[data-src="real"]').click());
await setDepth(660); await page.waitForTimeout(1600); await page.screenshot({path:'shots/23-real-660.png'});
console.log('source note:', (await page.evaluate(()=>document.querySelector('#scan-source-note').textContent)).slice(0,45));
await setDepth(2700); await page.waitForTimeout(1600); await page.screenshot({path:'shots/24-real-cmb.png'});
await page.evaluate(()=>document.querySelector('#datasrc button[data-src="synth"]').click());
await setDepth(0); await page.waitForTimeout(800);
